...
def plot_side_by_side_transition_matrices(novice_learner, expert_learner):
    """Plot novice and expert transition matrices side by side."""
    ...
    # Process both models
    learners = [novice_learner, expert_learner]
    axes = [ax1, ax2]
    ...
    for learner in learners:
        ...
        for i, source in enumerate(learner.states):
            for j, target in enumerate(learner.states):
                transition_matrix[i, j] = learner.transition_counts[source][target]
    ...
    # Create both plots
    for learner, ax, title in zip(learners, axes, titles):
        transition_matrix = np.zeros((len(learner.states), len(learner.states)))
        for i, source in enumerate(learner.states):
            for j, target in enumerate(learner.states):
                transition_matrix[i, j] = learner.transition_counts[source][target]
    ...
    #           "Comparison shows how meditation expertise affects state transitions.\n"
    #           "Note differences in meta-awareness generation and mind-wandering recovery.",
    #           ha='center', fontsize=11, style='italic')
              
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.15)
    plt.savefig('./results/plots/transition_matrices_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()